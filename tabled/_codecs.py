...
with ignore_import_errors:
    ...
    def mat_bytes_to_dataframe(mat_bytes: Union[bytes, io.BytesIO]) -> pd.DataFrame:
        ...
        Example:
            >>> with open('example.mat', 'rb') as f:
            ...     data_bytes = f.read()
            >>> df = mat_bytes_to_dataframe(data_bytes)
        ...
        if len(data_vars) > 1:
            print(f"Multiple variables found in .mat file: {list(data_vars.keys())}. Using the first one.")
    ...
    def _convert_cell_to_dataframe(data: np.ndarray, var_name: str) -> pd.DataFrame:
        ...
        if len(data.shape) == 2 and data.shape[0] > 0:
            try:
                ...
                if all(isinstance(_extract_scalar(data[0, j]), str) for j in range(data.shape[1])):
                    has_headers = True
                    headers = [_extract_scalar(data[0, j]) for j in range(data.shape[1])]
